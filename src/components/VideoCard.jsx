  ...
  return (
    <Link to={`/watch/${item.slug}`} className="group overflow-hidden rounded-xl border border-titanium/25 bg-card/55 backdrop-blur-sm transition duration-300 active:scale-[0.985] hover:-translate-y-1 hover:border-cyan/45">
      <div className="relative aspect-video overflow-hidden">
        ...
        <div className="absolute left-4 top-4 font-mono text-[9px] uppercase tracking-[0.2em] text-cyan/75">{item.id}</div>
      ...
      <div className="p-5">
        ...
        <div className="mt-2 text-xs text-seafoam/40">{formatViews(item.views)} views</div>
      </div>
    </Link>
  );
}
