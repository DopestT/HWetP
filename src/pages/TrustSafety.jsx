...
const SECTION_META = {
  ...
  report: { eyebrow: "Trust & safety", title: "Report content", intro: "Flag content or behavior for review. Production reports will enter a moderation queue with evidence, timestamps, and escalation status.", icon: Flag },
  takedown: { eyebrow: "Rights & removal", title: "Takedown request", intro: "Use this path for copyright, consent, impersonation, privacy, or other removal requests. Production handling will preserve evidence and route urgent safety issues separately.", icon: FileWarning },
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-abyss text-seafoam sonar-cursor">
      ...
      <main className="relative z-10 mx-auto max-w-[1200px] px-5 pb-20 pt-24 sm:px-6 md:px-12 md:pb-24 md:pt-32">
        ...
        <div className="mt-12 rounded-xl border border-cyan/15 bg-abyss/45 p-5 font-mono text-[9px] uppercase leading-relaxed tracking-[0.16em] text-seafoam/35 backdrop-blur-md">Pre-launch foundation. Forms are not connected to an external case-management system yet. Final legal and compliance text must be reviewed for the actual launch configuration.</div>
      </main>
...
function ReportForm({ params }) {
  ...
  if (sent) return <Success title="Report captured locally" body="No external report was sent. The production moderation queue will replace this placeholder behavior." />;
  return <form onSubmit={(event) => { event.preventDefault(); setSent(true); }} className="glass-panel rounded-2xl p-5 sm:p-6 md:p-8"><div className="grid gap-5 md:grid-cols-2"><Field label="Content reference"><input required defaultValue={ref || video} placeholder="Video ID or URL" className={inputClass} /></Field><Field label="Reason"><select required className={inputClass} defaultValue=""><option value="" disabled>Select reason</option><option>Consent concern</option><option>Copyright concern</option><option>Impersonation</option><option>Illegal or prohibited content</option><option>Harassment or abuse</option><option>Other</option></select></Field></div><Field label="What should we know?" className="mt-5"><textarea required rows={6} className={inputClass} placeholder="Describe the issue and include useful context." /></Field><Field label="Email for follow-up (optional)" className="mt-5"><input type="email" className={inputClass} placeholder="you@example.com" /></Field><button className="mt-6 min-h-12 w-full rounded-xl border border-cyan/50 bg-cyan/10 px-6 py-3 font-heading text-xs uppercase tracking-[0.18em] text-cyan transition hover:bg-cyan hover:text-abyss sm:w-auto sm:rounded-full">Submit report</button></form>;
}

function TakedownForm({ params }) {
  const [sent, setSent] = useState(false);
  const video = params.get("video") || "";
  if (sent) return <Success title="Request captured locally" body="No external takedown request was sent. Production will create a case ID, evidence record, and review status." />;
  return <form onSubmit={(event) => { event.preventDefault(); setSent(true); }} className="glass-panel rounded-2xl p-5 sm:p-6 md:p-8"><div className="grid gap-5 md:grid-cols-2"><Field label="Request type"><select required className={inputClass} defaultValue=""><option value="" disabled>Select request</option><option>Copyright / DMCA</option><option>Consent / intimate imagery</option><option>Identity / impersonation</option><option>Privacy</option><option>Other legal request</option></select></Field><Field label="Content URL or reference"><input required defaultValue={video} className={inputClass} placeholder="URL or content ID" /></Field><Field label="Your name"><input required className={inputClass} placeholder="Full name" /></Field><Field label="Contact email"><input required type="email" className={inputClass} placeholder="you@example.com" /></Field></div><Field label="Basis for the request" className="mt-5"><textarea required rows={6} className={inputClass} placeholder="Describe your rights, relationship to the content, and requested action." /></Field><label className="mt-5 flex items-start gap-3 text-sm text-seafoam/55"><input required type="checkbox" className="mt-1" /><span>I confirm the information provided is accurate to the best of my knowledge.</span></label><button className="mt-6 min-h-12 w-full rounded-xl border border-cyan/50 bg-cyan/10 px-6 py-3 font-heading text-xs uppercase tracking-[0.18em] text-cyan transition hover:bg-cyan hover:text-abyss sm:w-auto sm:rounded-full">Submit request</button></form>;
}
