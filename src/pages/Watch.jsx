...
export default function Watch() {
  ...
  const video = useMemo(() => VIDEOS.find((item) => item.slug === slug) || VIDEOS[0], [slug]);
  const related = VIDEOS.filter((item) => item.slug !== video.slug);

  useEffect(() => {
    setSaved(isFavorite(video.slug));
    addToHistory(video);
  }, [video]);

  const handleFavorite = () => setSaved(toggleFavorite(video));
        ...
        <div className="mt-7 grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section>
            <div className="relative aspect-video overflow-hidden rounded-xl border border-titanium/30 bg-card shadow-[0_0_60px_rgba(0,242,255,0.08)] sm:rounded-2xl">
              <div className={`absolute inset-0 bg-gradient-to-br ${video.tone}`} /><div className="absolute inset-0 isobaric-lines opacity-25" /><div className="pointer-events-none absolute left-1/2 top-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-cyan/10 blur-[90px]" />
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-center gap-3 bg-gradient-to-t from-abyss/95 via-abyss/55 to-transparent px-3 pb-3 pt-14 sm:gap-4 sm:px-5 sm:pb-4 sm:pt-16"><button className="text-seafoam/70 transition hover:text-cyan" aria-label="Play"><Play size={16} fill="currentColor" /></button><div className="h-1 flex-1 overflow-hidden rounded-full bg-seafoam/15"><div className="h-full w-[28%] rounded-full bg-cyan" /></div><span className="hidden font-mono text-[10px] text-seafoam/60 sm:inline">03:21 / {video.duration}</span><Volume2 size={16} className="hidden text-seafoam/60 sm:block" /></div>
            </div>

            <div className="mt-6 flex flex-col gap-5 border-b border-titanium/25 pb-7 md:flex-row md:items-start md:justify-between">
              <div><div className="font-mono text-[10px] uppercase tracking-[0.25em] text-cyan/75">{video.id} · surfaced now</div><h1 className="mt-2 font-heading text-[clamp(2rem,10vw,3rem)] font-bold uppercase tracking-[0.03em] text-seafoam md:text-5xl">{video.title}</h1><p className="mt-2 text-sm text-seafoam/45">{formatViews(video.views)} views · {video.duration} · Neutral production placeholder</p></div>
